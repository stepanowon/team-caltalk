  ...
  describe('회원가입 → 로그인 → 팀 생성 → 팀 참여 플로우', () => {
    it('전체 사용자 플로우가 정상적으로 작동해야 한다', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByRole('heading', { name: /로그인/i })
          ).toBeInTheDocument()
        },
        { timeout: 5000 }
      )
      ...
      await waitFor(
        () => {
          expect(
            screen.getByRole('button', { name: /팀 생성/i })
          ).toBeInTheDocument()
        },
        { timeout: 3000 }
      )
      ...
      await waitFor(
        () => {
          expect(
            screen.getByRole('dialog', { name: /팀 생성/i })
          ).toBeInTheDocument()
        },
        { timeout: 1000 }
      )
      ...
      const teamDescriptionInput = screen.getByRole('textbox', {
        name: /팀 설명/i,
      })
    ...
    it('다른 사용자로 팀 참여 플로우가 작동해야 한다', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByRole('button', { name: /팀 참여/i })
          ).toBeInTheDocument()
        },
        { timeout: 3000 }
      )
      ...
      await waitFor(
        () => {
          expect(
            screen.getByRole('dialog', { name: /팀 참여/i })
          ).toBeInTheDocument()
        },
        { timeout: 1000 }
      )

      const inviteCodeInput = screen.getByRole('textbox', {
        name: /초대 코드/i,
      })
  ...
  describe('에러 상황 처리', () => {
    ...
    it('잘못된 초대 코드로 팀 참여 시 에러를 처리해야 한다', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/유효하지 않은 초대 코드입니다/i)
          ).toBeInTheDocument()
        },
        { timeout: 3000 }
      )
    ...
    it('중복된 팀명으로 팀 생성 시 에러를 처리해야 한다', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/이미 존재하는 팀명입니다/i)
          ).toBeInTheDocument()
        },
        { timeout: 3000 }
      )
